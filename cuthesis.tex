...
In order to measure the significance of a trigger, approximately one hour of data either side of the on-source window is also analysed, called the \textit{off-source} data. The off-source data is split into trials of equal length to the on-source window. The false alarm probability is calculated as the fraction of off-source trials that have a higher energy than the highest energy on-source trigger. To create more trials and hence achieve a lower false alarm probability, timeslides are applied to the off-source to simulate more data. 

\subsection{O2 \xp Search}
...
The injections used for this analysis were circular sine-gaussian (CSG) and accretion disk instabilities (ADI) [\textbf{citations in O2 GRB paper}] waveforms. The CSG injections had a total radiated energy of $E_\text{GW} = 10^{-2}M_\odot c^2$ and a Q-factor of nine. \textbf{ASK PATRICK - is q factor just number of cycles?} The ADI waveforms are model the GW signal from instabilities in a magnetically suspended torus around a rapidly spinning black hole. These two waveforms are chosen as they have very different morphology and duration, with CSG waveforms lasting $\sim1$ second and ADI waveforms lasting for $\sim100$ seconds.

Of the 98 GRBs analysed, the only statistically significant signal was that of GRB 170817A, which was recovered with a false alarm probability of $3.1\times 10^{-4}$. The false alarm probability the 97 other GRBs analysed is plotted in figure \ref{fig:xpvalue}. A weighted binomial test of the 5\% most significant GRBs analysed by \xp in O2 gave a combined p-value of 0.75, indicating that there is no evidence for a sub-threshold population of weak GRB triggers. Plotted in \ref{fig:x ex dist} is the 90\% confidence exclusion distance for each analysed GRB apart from GRB 170817A. This is the distance at which 90\% of injected signals are more significant than the most significant on-source event. This can be used to rule out a burst event occurring within this exclusion distance for each GRB, which also limits the amount of energy that GRBs and be emitting as GWs. This can also be used as a measure of search sensitivity to burst signals, and comparison with the \xp results from O1 shows that the sensitivity is largely unchanged.
...
We analysed the 13 GRBs of our results set in the same manner as was done for the \xp results in O2. In figure \ref{fig:mva pvalues} we plot the p-values for each GRB, which can be compared to \ref{fig:xpvalue}. We see that the MVA has a bias towards low p-values. In particular, there are two p-values of about $\sim1\%$. Further investigation shows that these two low p-value events were ranked by \xp to be the most significant triggers in the on-source before vetoes were applied, but were then vetoed by the coherent cuts. One of the benefits of using MVA is that noise transients such as these should be down-weighted rather than simply cut, rather than the binary cut used by \xp. In these cases, it seems the MVA did not down-weight these events by enough. 
...
While these two triggers have suspiciously low p-values, it is worth comparing to the XTMVA analysis of GRB 170817A, as this was a real GW signal. In this analysis, the p-value of the most significant trigger was 0.0004095, a factor of twenty lower than the most significant other trigger in the 13 other analysed GRBs. This p-value corresponds to just one off-source trial having a more significant event, and is therefore at the limit of how low a p-value we can achieve with the number of timeslides we were using. The on-source for this event also had two other low p-value events, which time-frequency data show are different parts of the inspiral. Had the energy from these different sections been considered one trigger, then the significance would have been even greater. We will discuss how this might be achieved in section \ref{sec:mva future}. 
...
\subsubsection{Generalised Clustering}
Another tool that \xp has available that cannot be used in XTMVA at the moment is \textit{Generalised Clustering}. This is a change in the way \xp defines a trigger. Without using generalised clustering, triggers are groups of neighbouring pixels in the time-frequency maps such as in fig.\ref{fig:tfmap}. Generalised clustering allows these pixels to be separated by a user-specified number of pixels. This can boost the signal power of long triggers, as they tend to contain breaks in their time-frequency maps that causes \xp to report multiple short triggers for what is in fact one astrophysical event. This is what happened with the analysis of GW 170817 mentioned in section \ref{sec:mva results}, where the MVA reported three high significance triggers in a small time-frequency window. The downside to using generalised clustering is that it can cause noise triggers to be grouped together as well, boosting the power of noise. This then causes our sensitivity to short triggers to be reduced slightly. Experimenting with generalised clustering in \xp analysis analysis suggests that the benefits outweigh the costs, with an improvement of between 7\% and 50\% for long inspiral and adi waveforms at the cost of a decrease of 3\% to 9\% for short sine-gaussian waveforms. Experiments were done to see if generalised clustering would improve the sensitivity of XTMVA. We found that the 50\% injection scale upper limit improved for different waveforms saw roughly the same changes as \xp. However, for very large injections, the detection efficiency began to fall again. As well as this, very low amplitude injections, that should have not been detectable, were now being detected. To see the problem, we have plotted a detection efficiency curves for the NSBH injection set of an XTMVA analysis without generalised clustering in figure \ref{fig:no gc eff} and with generalised clustering in figure \ref{fig:gc eff}. It is clearly visible from the plots that using generalised clustering causes high energy injections to be missed while causing the false-detection of very low amplitude injections.

The problem at low amplitude effects every waveform set, while the problem at high amplitude seems to only effect longer waveforms, such as ADIs and inspirals, while the shorter waveforms, such as sine-gaussians, do not see a fall in their detection efficiency. This led us to believe that while both problems were related to generalised clustering, they were in fact two separate problems. We are confident that the reason the detection efficiency is dropping for many long waveforms is because the generalised clustering is making both signal and background triggers louder and longer, which increases the chance that the injection will be coincident in time or frequency with a background trigger. This will the cause the injection to be removed by the cleaning code mentioned in section \ref{sec:data-preprocessing}. To test this we looked at the distribution of triggers in the training set as a function of the number of pixels in the trigger. The results are shown in a box plot in figure \ref{fig:npixel box}. The plot shows that the training set with generalised clustering has a much higher ratio of small triggers, consistent with the cleaning explanation. 

This also helps explain the low amplitude detection efficiency problem, which we believe is caused by generalised clustering making background triggers louder and longer. This means that when a low amplitude injection is placed in the data, it is more likely to be clustered with noise. As there is a trigger coincident with the injection, the pipeline claims a detection. However, many of these low amplitude injections will have the same properties as noise. This is a very similar argument to the one that led us to introduce a threshold on the amplitude of signal injection in section \ref{sec:data-preprocessing}. We experimented with increasing the significance threshold required to be included in the training set, which improved the detection efficiency curve, but only at the expense of much worse sensitivity to short waveform and only marginal improvements in sensitivity to long waveforms than without using generalised clustering.

For these reasons, generalised clustering does not seem to be compatible with the MVA, despite the significant improvements it brings to \xp. To make generalised clustering work for XTMVA in future we could rearrange the pipeline so that cleaning happens before generalised clustering. This would prevent many long injections being removed from the training set. 
...
\begin{figure} % Example of including images
\begin{center}
\includegraphics[width=0.8\linewidth]{npixels_box.png}
\end{center}
\caption{\textbf{Number of Pixels per trigger, with and without Generalised Clustering.} Here we have a box plot of the number of pixels per trigger in the training set for XTMVA in a default (no generalised clustering) analysis and another analysis with generalised clustering. The plot has been truncated as both sets have a large tail of triggers with $\sim1000$ pixels. The plot shows that the generalised clustering training set has a much narrower distribution of trigger sizes. This is likely because generalised clustering increases the duration of background and signal triggers, making it more likely that an injection will be removed from the training set by cleaning. }
\label{fig:npixel box}
\end{figure}
