...
As well as the sensitivity improvement, XTMVA is also uses less CPU time than \xpfs In particular, using training an MVA classifier is much faster than calculating the optimal coherent cut with \xpfs The post-processing stage with \xp takes approximately 75 CPU hours, while the MVA takes about 18. Though it should be noted that the initial processing stage, where coherent statistics are calculated and triggers found, is still the most computationally intensive part of the process, taking about 650 hours. This part of the pipeline is identical between XTMVA and \xpfs
